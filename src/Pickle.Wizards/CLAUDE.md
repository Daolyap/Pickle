...
- JSON-only extensions not yet on the Abstractions models (`raw`, `warning`) are read by `WizardLoader` into a side
  table (`WizardSchema.IsRaw/GetWarning`); when Abstractions gains the properties, switch those accessors over.
